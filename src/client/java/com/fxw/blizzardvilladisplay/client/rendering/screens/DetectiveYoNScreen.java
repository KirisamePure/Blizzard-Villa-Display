...
import com.fxw.blizzardvilladisplay.networking.payload.DetectiveConfirmC2SPayload;
...
public class DetectiveYoNScreen extends Screen {
    public DetectiveYoNScreen(DetectiveConfig config) {
        super(Component.empty());
        this.config = config;
    }
    private DetectiveConfig config;
    ...
    public void init() {
        ...
        widgetConfirm = new ChooseCharWidget(
                ...
                Component.empty(),
                () -> {
                    ClientPlayNetworking.send(new DetectiveConfirmC2SPayload(config.status()));
                    this.startClosing();
                });
    ...
    private void renderBgTexture(GuiGraphics context, DetectiveConfig config) {
    ...
    private void renderConfirmtexture(GuiGraphics context, DetectiveConfig config) {
        long now = Util.getMillis();
        long elapsed = now - animationStart;
    ...
    private void renderCanceltexture(GuiGraphics context, DetectiveConfig config) {
        long now = Util.getMillis();
        long elapsed = now - animationStart;
