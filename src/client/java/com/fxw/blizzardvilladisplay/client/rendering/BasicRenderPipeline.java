...
public class BasicRenderPipeline implements ClientModInitializer {
    private static BasicRenderPipeline instance;
    ...
    // :::custom-pipelines:drawing-phase
    public static BasicRenderPipeline getInstance() {
        return instance;
    }
    ...
    private GpuBuffer upload(MeshData.DrawState drawParameters, VertexFormat format, MeshData builtBuffer) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < vertexBufferSize) {
            ...
            vertexBuffer = new MappableRingBuffer(() -> BlizzardVillaDisplay.MOD_ID + " example render pipeline", GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_MAP_WRITE, vertexBufferSize);
        }
    ...
    private static void draw(Minecraft client, RenderPipeline pipeline, MeshData builtBuffer, MeshData.DrawState drawParameters, GpuBuffer vertices, VertexFormat format) {
        ...
        try (RenderPass renderPass = RenderSystem.getDevice()
                ...
                .createRenderPass(() -> BlizzardVillaDisplay.MOD_ID + " example render pipeline rendering", client.getMainRenderTarget().getColorTextureView(), OptionalInt.empty(), client.getMainRenderTarget().getDepthTextureView(), OptionalDouble.empty())) {
